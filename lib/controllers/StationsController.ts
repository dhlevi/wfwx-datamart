...
export class StationsController extends Controller {
  ...
  @NoCache()
  @Cors({ origin: false })
  public async getStations (@Query('page') page: number, @Query('rows') rows: number, @Query('asGeojson') asGeojson: string, @Query('bbox') bbox: string, @Query('point') point: string, @Query('radius') radius: number) {
    try {
      page = page || 0
      rows = rows || 10
      ...
      return await stationService.getStations(page, rows, returnJson, bbox, point, radius)
    } catch (err) {
